...
<head>
    <meta charset="UTF-8">
    <title>Add Product — Ronde's Inventory System</title>
</head>
<body>
...
<div id="confirm-box" style="display:none; border:1px solid black; padding:10px; margin-bottom:10px; background:#fff8dc;">
    ...
    <button type="button" onclick="document.getElementById('add-form').submit()">Yes, Add</button>
    <button type="button" onclick="document.getElementById('confirm-box').style.display='none'">Cancel</button>
</div>

<form method="POST" action="<?= $baseUrl ?>/products" id="add-form">

    <label>Product Name:</label><br>
    <input type="text" name="name" value="<?= htmlspecialchars($old['name'] ?? '') ?>"><br>
    ...
    <label>SKU:</label><br>
    <input type="text" name="sku" value="<?= htmlspecialchars($old['sku'] ?? '') ?>"><br>
    ...
    <input type="text" name="stock" value="<?= htmlspecialchars($old['stock'] ?? '') ?>"><br>
    ...
    <input type="text" name="price" value="<?= htmlspecialchars($old['price'] ?? '') ?>"><br>
